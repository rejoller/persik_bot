...
def train_and_save_model4():
    try:
        ...
        no_spam_df["cleaned_text"] = no_spam_df["preprocessed_text"].apply(
            delete_stopwords
        )
        ...
        new_spam_data = pd.DataFrame(
            {"joined_text": spam_phrases_extended, "label": spam_labels}
        )
        ...
        joblib.dump(
            tfidf_vectorizer, os.path.join(VECT_DESTINATION, "tfidf_vectorizer.pkl")
        )
        joblib.dump(
            count_vectorizer, os.path.join(VECT_DESTINATION, "count_vectorizer.pkl")
        )
        ...
        joblib.dump(
            nb_classifier, os.path.join(MODELS_DESTINATION, "naive_bayes_model.pkl")
        )
        joblib.dump(
            nb_classifier1, os.path.join(MODELS_DESTINATION, "naive_bayes_model1.pkl")
        )
        ...
        joblib.dump(
            logreg, os.path.join(MODELS_DESTINATION1, "logistic_regression_model.pkl")
        )
        joblib.dump(
            logreg1, os.path.join(MODELS_DESTINATION1, "logistic_regression_model1.pkl")
        )
        logging.info(
            f"модель натренирована и сохранена в {VECT_DESTINATION} {MODELS_DESTINATION1} {MODELS_DESTINATION1}"
        )
